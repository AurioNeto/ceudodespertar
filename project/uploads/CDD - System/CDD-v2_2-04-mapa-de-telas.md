...
**Versão 0.3** · Autor: Aurio Neto · Data: agosto/2026 · Status: proposta para validação

> Alinhado aos **Documentos 2 e 3 na v2.2**.
>
> **0.1 → 0.2:** aplicadas as decisões de design Q1, Q2 e Q6 (§14.1).
> **0.2 → 0.3:** propagação de Q1 e Q2 concluída nos Documentos 2 e 3 (§14.3). A precedência normal entre os documentos está restabelecida.
...
### 14.3 Propagação para os Documentos 2 e 3 — **concluída**

As decisões Q1 e Q2 alteraram artefatos fora deste documento. A propagação foi feita e publicada como **Doc 2 v2.2** e **Doc 3 v2.2**; a precedência normal entre os documentos está restabelecida.

| Alteração | Documento | Seção | Situação |
|---|---|---|---|
| `StatusLancamento`: `RASCUNHO` → `A_CONFERIR` | Doc 2 v2.2 | §1.3, §1.10 (P1), §1.14, §5.1.1 | ✅ aplicada |
| `Lancamento.pendencias: Pendencia[]` + invariantes **L10** e **L11** | Doc 2 v2.2 | §1.3 | ✅ aplicada |
| Eventos `PendenciaRegistrada` / `PendenciaRespondida` | Doc 2 v2.2 | §1.13 | ✅ aplicada |
| Read model *Meus registros* | Doc 2 v2.2 | §1.14 | ✅ aplicada |
| `financeiro.lancamento.ler_proprios` — catálogo, matriz, seed e auditoria | Doc 3 v2.2 | §4.1, §5.1, §6.1, §10.2, §10.4 | ✅ aplicada |
| Casos de teste T16a–T16e | Doc 3 v2.2 | §11.2 | ✅ aplicada |
| Redação de `lancamento.registrar` e `.confirmar` | Doc 3 v2.2 | §4.1 | ✅ aplicada |

**Duas invariantes nasceram da propagação e não existiam quando este documento foi escrito.** Ambas afetam o desenho de `F-02` e `F-03`:

- **L10** — pendência **não é um estado**. O lançamento devolvido continua `A_CONFERIR`; o que muda é de quem o sistema espera ação. A tela não deve inventar um rótulo "devolvido" que o domínio não tem.
- **L11** — só o destinatário responde à pendência. Quem confere pode reabrir a pergunta, nunca respondê-la por suposição. `F-03` precisa desenhar isso como ausência de campo, não como campo desabilitado.

> **A lacuna do campo `pendencias` é um resultado do método, não um defeito dele.** A tela exigiu um campo que a especificação textual deixara implícito em prosa — exatamente o que o Doc 1 §8.1 prometia que o design-first produziria antes de o backend existir. A contrapartida honesta: L10 e L11 só apareceram quando o modelo foi tocado, o que confirma que a ida ao Doc 2 não era formalidade.

**Pendência residual:** o Documento 1 ainda se descreve como parte de um conjunto de três e menciona `RASCUNHO` em três passagens (§5.3, §9.2 e a nota de posicionamento do WhatsApp). São ajustes de redação, sem efeito sobre decisão alguma, a fazer na próxima revisão daquele documento.
...
2. **Revisar `F-02` e `F-03` à luz de L10 e L11** (Doc 2 v2.2 §1.3) — as duas invariantes nasceram da propagação e mudam o desenho: nada de rótulo "devolvido", nada de campo de resposta para quem confere.
...
*Documento derivado do Documento 1 (v2.1) e dos Documentos 2 e 3 (v2.2). Toda permissão citada existe no catálogo do Doc 3 §4; toda regra de tela citada corresponde a uma invariante do Doc 2. **Onde este documento diverge daqueles, prevalecem aqueles** — sem exceções pendentes.*
